...
void BSTree::Register()
{
	...
	cout << "\nENTER THE AGE        :";
	cin >> usr.age;
	cout << "\nAre you already vaccinated?" << endl;
	cout << "1) Yes" << "\n" << "2) No" << endl;
	...
	if (usr.first_dose)
	{
		cout << "\nDid you take the second dose?" << endl;
		cout << "1) Yes" << "\n" << "2) No" << endl;
		cin >> vaccinatedStatus;
		if (vaccinatedStatus == 1 || vaccinatedStatus == 2)
...
void BSTree::Remove(ll ID)
{
	...
	if ((node_to_be_deleted->left == 0) && (node_to_be_deleted->right == 0)) //deleting leaf node
	{
	...
	while (select == 1 || select == 2 || select == 3 || select==4 || select==5 || select==6 || select==7)
	{
		...
		else if (select == 3)
		{
			...
			if (!FirstWaitingList.isEmpty()) {
				...
				while (removeChoice == 1 && !FirstWaitingList.isEmpty())
				{
			...
			if (!SecondWaitingList.isEmpty()) {
				...
				cout << "2) NO." << endl;
				int removeChoice; cin >> removeChoice;
				while (removeChoice == 1 && !SecondWaitingList.isEmpty())
				{
